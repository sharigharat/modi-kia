...
export default function Logo({ className = "", dark = false }: { className?: string; dark?: boolean }) {
  return (
    <Link href="/#home" className={`group flex items-center gap-2.5 shrink min-w-0 ${className}`}>
      <Image
      ...
      <span className="leading-none flex flex-col gap-0.5 overflow-hidden min-w-0 shrink">
        <span className="flex flex-col min-w-0">
          <span className={`block font-sans text-sm font-bold tracking-normal truncate ${dark ? "text-white" : "text-black"}`}>
            Modi Kia
          </span>
        </span>
        <span className={`block text-[9px] font-medium uppercase tracking-[0.28em] truncate ${dark ? "text-white/60" : "text-muted"}`}>
